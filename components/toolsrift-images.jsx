...
const HOWTO = {
"image-resizer":"Upload an image, enter your target width and height (or lock the aspect ratio), and the tool resizes it and gives you a download link.",
"image-cropper":"Upload an image, drag the crop box to the area you want, and the tool exports just that region as a new image.",
"image-compressor":"Upload a PNG or JPG and set a quality level, and the tool compresses it and shows the new file size next to the original for comparison.",
"image-converter":"Upload an image and choose JPG, PNG, or WebP as the output format, and the tool converts it and gives you a download link.",
"image-rotator":"Upload an image and choose 90, 180, or 270 degrees, and the tool rotates it instantly and gives you a download link.",
"image-flipper":"Upload an image and choose horizontal or vertical, and the tool mirrors it and gives you a download link.",
"image-thumbnail":"Upload an image and set your target thumbnail size, and the tool generates a resized, compressed thumbnail ready to download.",
"image-grayscale":"Upload an image, and the tool converts it to black and white and gives you a download link.",
"image-brightness":"Upload an image and drag the brightness and contrast sliders, and the tool applies the adjustment live with a preview.",
"image-blur":"Upload an image and set the blur radius, and the tool applies a Gaussian blur and shows a live preview before you download.",
"image-sharpen":"Upload an image and set the sharpen strength, and the tool enhances edge detail and shows a live preview before you download.",
"image-invert":"Upload an image, and the tool inverts every color to its negative and gives you a download link.",
"image-sepia":"Upload an image, and the tool applies a vintage sepia tone and gives you a download link.",
"image-pixelate":"Upload an image and set the block size, and the tool applies a mosaic pixelation effect with a live preview.",
"image-emboss":"Upload an image, and the tool applies an emboss/relief effect and gives you a download link.",
"image-vignette":"Upload an image and set the vignette strength, and the tool darkens the edges with a live preview before you download.",
"image-watermark":"Upload an image, type your watermark text, and position and style it, and the tool overlays it and gives you a download link.",
"image-text-overlay":"Upload an image, type your text, and choose the font, size, color, and position, and the tool overlays it with a live preview.",
"image-caption":"Upload an image and type your caption text, and the tool adds a styled caption bar below the image, ready to download.",
"image-border":"Upload an image and choose a border color, width, and style, and the tool adds the frame with a live preview.",
"image-logo":"Upload your base image and a logo image, position and resize the logo, and the tool overlays it and gives you a download link.",
"image-color-picker":"Upload an image and click anywhere on it, and the tool reads the exact pixel color and shows its HEX, RGB, and HSL values.",
"image-dominant-colors":"Upload an image, and the tool analyzes the pixels and extracts the top dominant colors as a copyable palette.",
"image-metadata":"Upload an image, and the tool reads and displays its embedded EXIF metadata — camera settings, GPS, timestamps, and more.",
"image-info":"Upload an image, and the tool instantly shows its dimensions, file size, and format.",
"image-histogram":"Upload an image, and the tool renders a visual histogram of its brightness and color channel distribution.",
"placeholder-image":"Enter a width and height, and the tool generates a placeholder image at that exact size, ready to download or embed.",
"gradient-image":"Choose your gradient colors and direction, and set a size, and the tool renders a gradient background image ready to download.",
"solid-color-image":"Pick a color and enter a size, and the tool generates a solid color image at exactly that size, ready to download.",
"noise-texture":"Set the size and grain intensity, and the tool generates a random noise/grain texture image ready to download.",
"checkerboard":"Set the square size and colors, and the tool generates a checkerboard pattern image ready to download.",
"favicon-generator":"Upload any image, and the tool crops and resizes it into the standard favicon sizes, ready to download.",
"og-image-preview":"Enter your title, description, and image URL, and the tool renders a live preview of how your Open Graph social card will look when shared.",
"qr-code-generator":"Type or paste the text, URL, or data you want encoded, and the tool renders a QR code image you can download.",
"barcode-generator":"Enter your value and choose a barcode format (Code128, EAN, UPC, and more), and the tool renders a scannable barcode image to download.",
"avatar-generator":"Enter a name or initials and pick a background color, and the tool generates a letter avatar image ready to download.",
"image-base64":"Upload an image, and the tool encodes it into a base64 data URI you can copy and paste directly into CSS or HTML.",
"base64-to-image":"Paste a base64 data URI, and the tool decodes it back into a viewable, downloadable image.",
"svg-to-png":"Upload or paste an SVG, set your target size, and the tool rasterizes it into a downloadable PNG.",
"image-to-svg":"Upload an image, and the tool traces its shapes into a basic vector SVG you can download.",
"sprite-sheet":"Upload multiple images, and the tool arranges them into a single sprite sheet image ready to download.",
"image-splitter":"Upload an image and set the number of rows and columns, and the tool slices it into equal grid pieces you can download.",
"image-collage":"Upload 2 to 4 images and choose a layout, and the tool combines them into a single collage image ready to download.",
"pdf-to-image":"Upload a PDF, and the tool extracts each page as a separate image you can download.",
"image-to-pdf":"Upload a JPG or PNG, choose A4 or Letter page size, and the tool wraps it into a downloadable PDF.",
"screenshot-taker":"Click capture, and the tool takes a screenshot of the current page and gives you a downloadable image.",
"exif-reader":"Upload a photo, and the tool reads and displays its full EXIF data — camera model, exposure settings, GPS coordinates, and more.",
"exif-remover":"Upload an image, and the tool strips all embedded metadata and gives you a clean copy to download — useful before sharing photos publicly.",
"dpi-checker":"Upload an image, and the tool reads its embedded DPI/PPI value and reports whether it's suitable for print or screen use.",
"color-space":"Upload an image or enter RGB values, and the tool converts them to a simulated CMYK equivalent for print-color reference.",
"image-threshold":"Upload an image and set a luma threshold, and the tool binarizes it into pure black and white with a live preview.",
"image-posterize":"Upload an image and set the number of color levels per channel, and the tool reduces the color range with a live preview.",
"image-contrast":"Upload an image and drag the contrast slider, and the tool applies the adjustment with a live preview before you download.",
"image-gamma":"Upload an image and adjust the gamma value, and the tool remaps the midtone brightness with a live preview.",
"image-saturation":"Upload an image and drag the saturation slider, and the tool boosts or mutes the color intensity with a live preview.",
"image-hue-rotate":"Upload an image and drag the hue slider, and the tool shifts every color around the hue wheel with a live preview.",
"image-tint":"Upload an image and pick a tint color, and the tool multiplies it across the image for a color-wash effect with a live preview.",
"image-duotone":"Upload an image and pick two colors, and the tool maps the image's brightness onto that two-color gradient with a live preview.",
"image-color-replace":"Upload an image, pick the color to replace and the new color, and set a tolerance, and the tool swaps matching pixels with a live preview.",
"image-round-corners":"Upload an image and set the corner radius, and the tool rounds the corners with a transparent background, ready to download.",
"image-circle-crop":"Upload an image, and the tool crops it into a circle with a transparent background, ready to download.",
"image-padding":"Upload an image, set the padding amount and a background color, and the tool adds a colored margin around it, ready to download.",
"image-ascii-art":"Upload an image and set the output width, and the tool converts it into copyable ASCII text art built from the image's brightness values.",
};
