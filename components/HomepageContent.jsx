...
const PRINCIPLES = [
  ...
  { icon: '🚀', title: 'Always growing', body: `${TOOLS_LABEL} tools today, 1,600+ planned. New tools added weekly based on user requests — email us what you wish existed and we will probably build it.` },
...
const FAQS = [
  ['Is ToolsRift really completely free?', `Yes. Every one of the ${TOOLS_PLUS} tools is free with no daily limit, no sign-up, no paywall and no premium tier. ToolsRift is supported by non-intrusive display advertising — that is what funds the platform and lets us keep everything free for everyone, forever.`],
    ...
    <>
      <Head>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "description": `ToolsRift offers ${TOOLS_PLUS} free online tools across ${TOTAL_CATEGORIES} categories — text, PDF, image, code, calculators, generators and more. Every tool runs in your browser. No sign-up.`,
          "publisher": {
            "@type": "Organization",
            "name": "ToolsRift",
            "url": "https://toolsrift.com",
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 920, margin: '0 auto', padding: '0 24px 64px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
            {[
              { num: '01', icon: '🔍', title: 'Find the tool', body: `Browse ${TOTAL_CATEGORIES} categories or use the search to find one of ${TOOLS_PLUS} free tools. Each category page has rich descriptions, FAQs and use-case examples to help you pick the right one.` },
        ...
        <div style={{ maxWidth: 820, margin: '0 auto', padding: '0 24px 64px' }}>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.borderLight}`, borderRadius: 16, padding: 'clamp(28px,4vw,40px)' }}>
            ...
            <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.85, margin: '0 0 18px' }}>
              We were tired of it. So we started building the version of those tools we wanted — fast, clean, no sign-up, no upload, free forever. One tool became ten. Ten became a hundred. Today there are {TOOLS_PLUS} tools across {TOTAL_CATEGORIES} categories, and we are still adding more every week based on what people email us asking for.
            </p>
