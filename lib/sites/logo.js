...
function networkMarkSvg(b, size = 72, { id = 'nm', chip = true } = {}) {
  ...
  const glyph = chip ? GLYPHS[b.logo && b.logo.glyph] : null;
  if (chip && !glyph) throw new Error(`No glyph "${b.logo && b.logo.glyph}" for brand ${b.id}`);
...
/** The hub (toolsrift.com) drawn with the same badge, no category chip. */
const HUB_BRAND = {
  id: 'hub', siteName: 'ToolsRift', wordmark: ['Tools', 'Rift'],
  palette: { primary: '#3B82F6', primaryDark: '#1A6FD8', accent2: '#38BDF8', textOnPrimary: '#ffffff' },
  logo: null,
};
...
module.exports = { networkMarkSvg, networkLogoSvg, wordmarkLayout, HUB_BRAND };
