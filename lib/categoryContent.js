...
const RELATED = {
  ...
  random:      { name: 'Randomizers & Games',href: '/random',       desc: 'Spinner wheel, dice roller, random picker, magic 8 ball and more.' },
  audio:       { name: 'Audio Tools',        href: '/audio',        desc: 'Trim, convert, record and normalize audio plus text-to-speech.' },
  office:      { name: 'Office & Productivity', href: '/office',    desc: 'vCards, calendar invites, QR codes, notes, labels and signatures.' },
  data:        { name: 'Charts & Data',      href: '/data',         desc: 'Chart makers, CSV cleaning, pivots and quick data statistics.' },
  study:       { name: 'Study & Education',  href: '/study',        desc: 'Flashcards, quizzes, citations, periodic table and study timers.' },
  video:       { name: 'Video Tools',        href: '/video',        desc: 'Trim, convert, compress, GIF and record video — all in-browser.' },
}
...
const CONTENT = {
  ...
  /* ──────────────── RANDOM ──────────────── */
  random: {
    categoryName: 'Randomizers & Games',
    categorySlug: 'random',
    heroTagline: 'Spinner wheel, dice, random pickers and decision games — all in your browser',
    intro: [
      "Sometimes the fairest way to decide is to let chance do it. Whether you are picking a giveaway winner, splitting a class into teams, choosing where to eat, rolling for a critical hit in Dungeons & Dragons, or just settling an argument with heads or tails, a good randomizer removes bias and makes the choice fun. ToolsRift offers 10 free randomizer and game tools that run entirely in your browser, with no sign-up and nothing uploaded anywhere.",
      "The collection covers the tools people reach for most: an animated spinner wheel of names for draws and classrooms, a random team generator that splits people into balanced groups, a multi-die RPG dice roller for tabletop games, random number and random letter generators with filters, a batch coin flipper, a printable bingo card maker, the classic Magic 8 Ball, and a simple yes-or-no decision maker.",
      "Every tool uses your browser's cryptographically-secure random generator, so results are genuinely unpredictable and unbiased — every name, number and outcome has an exactly equal chance. There are no accounts, no limits, and no waiting on a server."
    ],
    whyToolsRift: [
      "Many random-picker and wheel-of-names sites hide the good features behind sign-up walls, cap how many entries you can add, or plaster the result screen with pop-up ads right when you are trying to announce a winner. ToolsRift keeps every randomizer completely free with unlimited entries and unlimited spins, flips and rolls. Nothing is gated, and there is never a daily quota.",
      "Randomness quality matters too. Basic tools use Math.random(), which is a low-quality pseudo-random generator. ToolsRift uses crypto.getRandomValues — the same secure source used for cryptographic keys — so draws are provably fair and free of the subtle bias that cheaper generators introduce. And because everything runs on your device, your list of names or entries is never sent anywhere."
    ],
    howToUse: [
      { title: 'Pick a randomizer', body: 'Open the Randomizers & Games dashboard and choose the tool you need — spinner wheel, team generator, dice roller, number generator, coin flip, bingo, Magic 8 Ball and more.' },
      { title: 'Add your entries or options', body: 'Type names or options one per line for the wheel, picker and team generator, or set a range and count for the number generator. Sensible examples are pre-filled so you can try instantly.' },
      { title: 'Spin, roll or flip', body: 'Press the action button and watch the animated result. The spinner wheel eases to a stop on a winner; dice tumble to a total; the coin flipper reports heads and tails counts.' },
      { title: 'Copy or go again', body: 'Copy the result with one click, remove a winner and re-spin for multiple rounds, or generate a fresh outcome as many times as you like — there are no limits.' }
    ],
    useCases: [
      'Teachers picking a random student or splitting the class into groups',
      'Streamers and creators drawing giveaway and raffle winners live',
      'Tabletop and D&D players rolling multiple polyhedral dice at once',
      'Friends deciding where to eat or which movie to watch',
      'Managers randomly assigning tasks or ordering a stand-up',
      'Party hosts generating printable bingo cards for game night',
      'Anyone settling a decision with a fair coin flip or yes/no answer'
    ],
    faqs: [
      ['Are these randomizers really fair?', 'Yes. Every tool draws from your browser\'s crypto.getRandomValues generator, the same cryptographically-secure source used for encryption keys. That means each name, number or outcome has an exactly equal chance and there is no detectable bias, unlike tools built on the weaker Math.random().'],
      ['Do I need an account or is there a limit?', 'No account and no limits. Every randomizer is free with unlimited entries, spins, rolls and flips. There is no daily quota, no paywall and no sign-up — the platform is supported by non-intrusive ads.'],
      ['Is my list of names or entries uploaded anywhere?', 'No. Every tool runs entirely in your browser using JavaScript and the Canvas API. Your names, options and results never leave your device and are never stored on a server — you can confirm this in your browser\'s network tab.'],
      ['Can I use the spinner wheel for a giveaway?', 'Absolutely. The wheel of names is built for exactly that — add every entry, spin for a provably-fair winner, then remove that winner and spin again to draw multiple prizes. It works great on a shared screen during a live stream or event.'],
      ['Do the tools work on my phone?', 'Yes. All 10 randomizers are responsive and work on any modern phone, tablet or desktop browser with no app to install. The spinner wheel and dice animations are touch-friendly and scale to your screen.']
    ],
    related: pick('everyday', 'generators', 'mathcalc', 'business', 'devtools'),
  },
  ...
  audio: {
    ...
    categorySlug: 'audio',
    heroTagline: 'Trim, convert, record and generate audio — entirely inside your browser',
    intro: [
    ...
    howToUse: [
      { title: 'Choose your audio tool', body: 'Open the Audio Tools dashboard and pick the job you need — trim, convert, merge, normalize, record, text-to-speech, or one of the analysis and generator tools.' },
      { title: 'Load your file (or record one)', body: 'Select an audio file from your device, or drag and drop it onto the page. Recording tools ask your browser for microphone permission instead, which you can revoke at any time.' },
      { title: 'Adjust and preview', body: 'Set your trim points, output format, volume target or speed, then preview the result before exporting. Nothing is committed until you export, so you can experiment freely.' },
      { title: 'Export and download', body: 'Download the processed audio straight to your device. The file is generated locally in your browser, so the download is instant with no upload or waiting.' }
    ],
    useCases: [
    ...
    faqs: [
      ['Is my audio uploaded to a server?', 'No. Every audio tool here runs entirely in your browser using the Web Audio API and MediaRecorder. Your file is read locally, processed locally and exported locally — it is never transmitted, and you can confirm this yourself in your browser\'s network tab.'],
      ['What audio formats are supported?', 'You can open any format your browser can decode, which in practice covers MP3, WAV, OGG, M4A/AAC, FLAC and WebM. Export options depend on what your browser can encode — WAV is always available, and most modern browsers also support WebM and OGG.'],
      ['Is there a file size or length limit?', 'There is no limit imposed by us because nothing is uploaded. The practical ceiling is your device\'s available memory, so very long files (over an hour) may be slow on an older phone but are usually fine on a desktop.'],
      ['Does the voice recorder need permission?', 'Yes. Your browser will ask for microphone access the first time you use a recording tool, and the recording stays on your device. You can revoke that permission at any time in your browser settings, and nothing is captured while the recorder is stopped.'],
      ['Do these tools work offline?', 'Largely yes. Once the page has loaded, the trimming, conversion, analysis and generator tools keep working without a connection because all the processing code is already in your browser.']
    ],
    related: pick('images', 'pdf', 'data', 'office', 'devtools'),
  },
  ...
  office: {
    ...
    categorySlug: 'office',
    heroTagline: 'vCards, calendar invites, QR codes, notes and printables — no account required',
    intro: [
      "Office work is full of small formatting jobs that are annoying out of proportion to their size: building a contact card someone can actually scan, producing a calendar invite that opens correctly in Outlook and Google Calendar, printing a sheet of address labels, capturing a signature as a transparent PNG, or sharing a WiFi password without reading it out letter by letter. ToolsRift collects those jobs into a single set of free tools that run in your browser.",
      "Each tool produces a real, standards-compliant file — a .vcf contact card, an .ics calendar event, a print-ready label sheet, a PNG signature — rather than locking the output behind an account or a watermark. Because the files are generated locally, the contact details, meeting information and personal data you type never touch a server.",
      "The set spans contact and sharing tools, planning tools like notes, to-do lists and kanban boards that save to your own browser storage, and print-and-document helpers for labels, certificates, letterheads and envelopes."
    ],
    whyToolsRift: [
      "The typical free QR-code or vCard site tracks the codes it generates, expires them after a trial, or routes scans through its own redirect domain so it can measure them — which also means the code stops working if that company disappears. Ours generate a plain, permanent, self-contained code that encodes your data directly with no redirect and no tracking.",
      "Notes, task lists and boards save to your browser's own local storage, so they persist between visits without an account and without syncing your content anywhere. Everything is free, with no quotas and no upsell."
    ],
    howToUse: [
      { title: 'Pick the tool for the job', body: 'Open the Office & Productivity dashboard and choose what you need — vCard, calendar invite, QR code, notes, labels, signature pad or certificate maker.' },
      { title: 'Fill in your details', body: 'Type the contact, event or document details into the form. Most tools show a live preview so you can see exactly what the finished file or print sheet will look like.' },
      { title: 'Check the preview', body: 'Confirm the layout, spelling and formatting. Print-oriented tools show real page boundaries so nothing is cut off when it reaches the printer.' },
      { title: 'Download, print or copy', body: 'Export the .vcf, .ics, PNG or PDF-ready page directly to your device, or send it straight to your printer from the preview.' }
    ],
    useCases: [
      'Sales and field staff sharing a scannable contact card at events',
      'Event organisers sending calendar invites that open in any app',
      'Cafés and offices printing a WiFi QR code for guests',
      'Small businesses printing address and product label sheets',
      'Anyone capturing a signature as a transparent PNG for documents',
      'Teams running a quick private kanban board with no tool sign-up',
      'Schools and clubs printing certificates and name badges'
    ],
    faqs: [
      ['Do the QR codes ever expire?', 'No. The codes are static — your data is encoded directly into the image itself rather than pointing at a short link on our servers. That means the code keeps working forever, cannot be tracked by us, and does not depend on ToolsRift staying online.'],
      ['Where are my notes and tasks stored?', 'In your browser\'s local storage on your own device, not on any server. That keeps them completely private, but it also means they are tied to that browser — clearing your browser data will remove them, so export anything you want to keep.'],
      ['Will the calendar invite work in Outlook and Google Calendar?', 'Yes. The tool generates a standard .ics file following the iCalendar specification, which Outlook, Google Calendar, Apple Calendar and virtually every other calendar application can open and import.'],
      ['Can I use these documents commercially?', 'Yes. Anything you generate is yours to use however you like, including commercially, with no attribution required and no licence fee.'],
      ['Is my contact information uploaded anywhere?', 'No. Every field you type is processed locally in your browser to build the file, and nothing is sent to a server, logged or stored by us.']
    ],
    related: pick('business', 'generators', 'images', 'data', 'everyday'),
  },

  /* ──────────────── DATA ──────────────── */
  data: {
    categoryName: 'Charts & Data Tools',
    categorySlug: 'data',
    heroTagline: 'Turn raw numbers into clean charts and tidy CSV files — privately',
    intro: [
      "Most people who need a chart do not need a spreadsheet suite. They have a short list of numbers — monthly sales, survey answers, test results, project hours — and they want a clean, readable graphic they can drop into a report, a slide or a post. ToolsRift's chart makers take pasted data and produce a properly labelled bar, line, pie, donut, area or scatter chart that you can download as SVG or PNG.",
      "The other half of the collection handles the unglamorous work that comes before the chart: opening a CSV to actually look at it, removing duplicate rows, splitting a huge export into manageable pieces, merging files, extracting a single column, fixing inconsistent formatting, and summarising the result with quick statistics or a pivot.",
      "Because charts are exported as SVG, they stay perfectly sharp at any size — in a printed report, on a projector, or on a high-density phone screen — instead of turning into a blurry screenshot."
    ],
    whyToolsRift: [
      "Uploading a CSV to a free online chart site means handing over whatever that file contains, which is frequently customer names, salaries, medical figures or unreleased financials. These tools never upload anything: your data is parsed and rendered inside your browser, which makes them safe to use on files you could not legally or sensibly send to a third party.",
      "There is no account, no watermark on exports, no row cap, and no paid tier that unlocks the useful features. Charts come out clean and neutral by default, with sensible colours and readable labels, so they fit into a document without extra editing."
    ],
    howToUse: [
      { title: 'Choose a chart or data tool', body: 'Open the Charts & Data dashboard and pick a chart type, a CSV utility, or one of the analysis tools depending on what you are trying to produce.' },
      { title: 'Paste or load your data', body: 'Paste rows directly, or load a CSV file from your device. The tools accept comma, tab and semicolon separated data and detect headers automatically.' },
      { title: 'Label and style', body: 'Set the title, axis labels and colours. Every change re-renders the preview live so you can see the finished chart as you adjust it.' },
      { title: 'Export or copy', body: 'Download the chart as an SVG for print-quality output or a PNG for quick sharing, or copy cleaned data back out for use elsewhere.' }
    ],
    useCases: [
      'Students charting experiment results for a lab report',
      'Small business owners visualising monthly sales without Excel',
      'Analysts previewing and cleaning a CSV export before importing it',
      'Teachers turning class results into a readable graphic',
      'Writers and bloggers making a simple chart for an article',
      'Anyone splitting a very large CSV into smaller importable chunks',
      'Teams deduplicating and merging exported contact or product lists'
    ],
    faqs: [
      ['Is my CSV or data uploaded anywhere?', 'No. Files are read with the browser\'s FileReader API and parsed in memory on your own device. Nothing is transmitted to a server, which makes these tools safe for confidential financial, medical or customer data.'],
      ['What is the difference between the SVG and PNG export?', 'SVG is a vector format that stays perfectly sharp at any size and can be re-edited in design software, so it is best for print and slides. PNG is a fixed-resolution image that is easier to paste into chat, email or a document that does not accept SVG.'],
      ['How large a file can I open?', 'There is no limit set by us. Very large files (hundreds of megabytes) depend on your device\'s memory, so the CSV splitter is the better route for those — it breaks a large export into smaller pieces your other software can actually import.'],
      ['Can I use these charts commercially?', 'Yes. Charts you generate are yours with no watermark, no attribution requirement and no licence fee, including for commercial reports and published material.'],
      ['Do I need to know a formula language?', 'No. The tools are form-based — you paste data, pick options and read the result. There are no formulas to write and no spreadsheet syntax to learn.']
    ],
    related: pick('formatters', 'json', 'financecalc', 'mathcalc', 'office'),
  },

  /* ──────────────── STUDY ──────────────── */
  study: {
    categoryName: 'Study & Education Tools',
    categorySlug: 'study',
    heroTagline: 'Flashcards, citations, quizzes and reference tables for students and teachers',
    intro: [
      "Studying well is mostly about doing a few unglamorous things consistently: testing yourself instead of rereading, spacing your revision out, formatting your references correctly, and keeping a clear picture of what you still do not know. ToolsRift's study tools cover those tasks with free, no-signup utilities that work on a phone between classes as well as on a laptop.",
      "The collection includes a flashcard maker that turns any list of terms and definitions into flippable, shuffleable cards, a quiz generator, study and revision timers, grade calculators that tell you exactly what you need on the final, and a citation generator covering APA, MLA and Chicago styles for books, journals and websites.",
      "Alongside those sit the reference tools students reach for constantly: an interactive periodic table, physical constants, SI prefixes, the Greek alphabet, a molar mass calculator and a chemical equation balancer."
    ],
    whyToolsRift: [
      "The big flashcard and study platforms increasingly gate basic features — limiting how many cards you can make, putting self-testing modes behind a subscription, or requiring an account before you can study a set you typed yourself. Every tool here is free and works immediately, with no account and no card limit.",
      "Nothing you type is uploaded. Your card sets, notes and answers stay in your own browser, which matters when you are revising from material your institution has not licensed for redistribution. The tools are also fast and light enough to work on an old phone and to keep running offline once loaded."
    ],
    howToUse: [
      { title: 'Pick your study tool', body: 'Open the Study & Education dashboard and choose what you need — flashcards, a quiz, a citation, a grade calculator, a timer or a reference table.' },
      { title: 'Enter your material', body: 'Paste your terms and definitions, your source details, or your marks. Most tools accept a simple one-per-line format so you can paste straight from your notes.' },
      { title: 'Study, calculate or format', body: 'Flip and shuffle your cards, run the quiz, or read off the calculated grade or formatted citation. Results update as you change your input.' },
      { title: 'Print, copy or export', body: 'Print a flashcard set for offline revision, copy a finished citation into your document, or export your material to keep it.' }
    ],
    useCases: [
      'Students making flashcards for vocabulary, anatomy or formulas',
      'Anyone working out the mark they need on a final to hit a target grade',
      'Undergraduates formatting an APA or MLA reference list',
      'Teachers generating quick practice quizzes and worksheets',
      'Chemistry students balancing equations and computing molar mass',
      'Learners running spaced revision sessions with a study timer',
      'Tutors printing reference tables for a class handout'
    ],
    faqs: [
      ['Do I need an account to make flashcards?', 'No. You can create, shuffle, study and print an unlimited number of cards without signing up. Sets are saved in your own browser\'s storage, so they are still there when you come back on the same device.'],
      ['Are the citation formats accurate?', 'The generator follows the published rules for APA 7th edition, MLA 9th edition and Chicago style for the common source types. Always check the result against your institution\'s own style guide, since departments frequently apply local variations.'],
      ['Is my study material private?', 'Yes. Everything you type stays in your browser and is never uploaded, so you can safely enter material from copyrighted textbooks, lecture slides or unpublished notes.'],
      ['Can teachers use these tools in class?', 'Yes, freely and with no licence. Teachers commonly use the quiz generator, flashcard maker, worksheet tools and reference tables for classwork, and everything prints cleanly for handouts.'],
      ['Do the tools work on a phone?', 'Yes. All the study tools are responsive and designed for phone use, which is where most short revision sessions actually happen. Nothing needs to be installed.']
    ],
    related: pick('mathcalc', 'text', 'everyday', 'data', 'converters2'),
  },
  ...
  video: {
    ...
    categorySlug: 'video',
    heroTagline: 'Trim, convert, compress and record video — entirely inside your browser',
    intro: [
      "Most free online video tools work the same way: you upload your clip, wait in a queue behind other people's files, and download a capped or watermarked result — and you have no real idea how long that file sits on their server afterwards. ToolsRift's video tools skip that entire process. They run on WebAssembly builds of FFmpeg loaded directly into your browser, so trimming, converting and compressing a video never involves uploading it anywhere.",
      "That matters for video more than almost any other file type, because video is exactly what people are least willing to hand to a stranger's server — a client's unreleased ad, a family recording, screen capture of confidential work, or footage that is simply too large to wait on a slow upload for. Here, processing starts the moment you choose a file and runs at the speed of your own device.",
      "The collection covers the everyday jobs: trimming and merging clips, converting between MP4, WebM and MOV, compressing a file down to a shareable size, extracting the audio track or individual frames, converting video to an animated GIF, rotating and cropping, muting, adjusting playback speed, adding a watermark, and recording your webcam or screen directly in the page."
    ],
    whyToolsRift: [
      "Typical free video converters cap file length at a few minutes, add a watermark unless you subscribe, or simply fail silently on anything over a modest size because their server queue timed out. Because there is no server here, there is no queue and no arbitrary length cap beyond what your own device's memory can hold.",
      "Everything is free with no sign-up and no watermark. The trade-off is honest: in-browser video processing is slower than a dedicated desktop app or a powerful cloud render farm, especially on an older laptop or a phone, because your own hardware is doing the work instead of a data-center GPU. For a quick trim, convert or compress, that trade is easily worth never uploading the file at all."
    ],
    howToUse: [
      { title: 'Pick your video tool', body: 'Open the Video Tools dashboard and choose the job you need — trim, convert, compress, extract audio, make a GIF, or record your screen or webcam.' },
      { title: 'Load your video', body: 'Select a file from your device or drag it onto the page. The first use of any tool downloads the FFmpeg engine to your browser, which takes a few seconds on a normal connection and is then cached for the rest of your session.' },
      { title: 'Adjust and preview', body: 'Set your trim points, output format, target size or crop, and preview the change before exporting. Processing happens locally, so larger files simply take longer rather than costing more.' },
      { title: 'Export and download', body: 'Download the finished file straight to your device. Nothing is uploaded at any point, so the download is the only network-free step in the whole process — wait, there is no network step at all.' }
    ],
    useCases: [
      'Content creators trimming a long screen recording down to a clip',
      'Marketers converting an MP4 to a WebM for a faster-loading website',
      'Anyone compressing a phone video small enough to email or message',
      'Social media managers turning a short clip into a looping GIF',
      'Podcasters extracting just the audio track from a recorded video call',
      'Teams recording a quick screen-share walkthrough with no separate software',
      'Editors rotating, muting or watermarking a clip before sharing it'
    ],
    faqs: [
      ['Is my video uploaded to a server?', 'No. Every video tool here runs entirely in your browser using FFmpeg compiled to WebAssembly. Your file is read locally, processed locally and exported locally — you can confirm this yourself by checking your browser\'s network tab while a tool runs.'],
      ['Why does the first tool I open take a moment to load?', 'The FFmpeg engine itself is a sizeable WebAssembly download, so the very first video tool you open on a visit fetches it once. After that it is cached by your browser, so switching between video tools on the same visit is fast.'],
      ['Is there a file size or length limit?', 'There is no limit set by us, because nothing is uploaded. The real ceiling is your own device\'s available memory — a modern laptop handles a feature-length file without trouble, while a low-memory phone may struggle with anything very large.'],
      ['Why is browser video processing slower than desktop software?', 'WebAssembly runs close to native speed but still trails a purpose-built desktop encoder or a cloud render farm with dedicated hardware. For everyday trims, conversions and compressions the difference is a matter of seconds to a couple of minutes — a fair trade for never uploading the file.'],
      ['Do the recording tools need permission?', 'Yes. Your browser will ask for camera, microphone or screen-sharing permission the first time you use a recording tool, and the recording stays on your device throughout. You can revoke that permission at any time in your browser settings.']
    ],
    related: pick('images', 'audio', 'pdf', 'data', 'office'),
  },
