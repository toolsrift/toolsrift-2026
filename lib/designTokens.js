...
// ── Site-aware base palette ────────────────────────────────────────────────
// The hub keeps the original ToolsRift palette. A standalone network site
// (NEXT_PUBLIC_SITE_ID=pdf, …) swaps in its own base hue from
// lib/sites/brands.js so every shared layout re-skins itself automatically.
function rgba(hex, a) {
  const h = hex.replace('#', '');
  return `rgba(${parseInt(h.slice(0, 2), 16)},${parseInt(h.slice(2, 4), 16)},${parseInt(h.slice(4, 6), 16)},${a})`;
}
const P = SITE.brand ? SITE.brand.palette : null;

export const COLORS = {
  bg:          P ? P.bg        : '#06090F',
  bgRaised:    P ? P.bgRaised  : '#0B0F18',
  surface:     P ? P.surface   : '#0D1117',
  surface2:    P ? P.surface2  : '#111827',
  surface3:    P ? P.surface3  : '#161E2E',
  ...
  dim:         '#64748B',
  faint:       '#475569',
  navBg:       P ? rgba(P.bg, 0.85) : 'rgba(6,9,15,0.85)',
  navBgSolid:  P ? rgba(P.bg, 0.92) : 'rgba(6,9,15,0.92)',
  navBgSoft:   P ? rgba(P.bg, 0.75) : 'rgba(6,9,15,0.75)',
...
// ── Border radius scale ─────────────────────────────────────────────────────
const R = SITE.brand ? SITE.brand.shape.radius : 10;
export const RADIUS = {
  xs:   Math.max(2, Math.round(R * 0.4)),
  sm:   Math.max(3, Math.round(R * 0.6)),
  md:   R,
  lg:   R + 4,
  xl:   R + 8,
  '2xl': R + 12,
