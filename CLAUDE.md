...
- **IndexNow key file**: `public/509a62672848f5997b1eb6f154172d3a.txt` — proves domain ownership to IndexNow. NEVER delete or move it; the key is hardcoded in both `scripts/submit-indexnow.js` and `pages/api/cron/submit-search-engines.js`. If it's ever regenerated, update the key in both places and the filename together.
- **Manual/per-change push**: `node scripts/submit-indexnow.js` reads `public/sitemap.xml` and POSTs all URLs to `api.indexnow.org`, which fans out to Bing, Yandex, Seznam.cz, Naver, and Yep. Google does NOT consume IndexNow.
- **Google**: has no general-purpose instant-index API — the "Indexing API" is restricted by Google's terms to JobPosting/BroadcastEvent pages only; do not use it for tool pages. The legitimate automation is the Search Console API's `sitemaps.submit` (recrawl-request, not a guarantee of indexing), via `scripts/submit-google-sitemap.js`. Requires a Google Cloud service account (Search Console API enabled) added as a "Full" user on the `toolsrift.com` Search Console property, with its JSON key stored in the `GOOGLE_SERVICE_ACCOUNT_KEY` env var (Vercel dashboard + local `.env.local`, gitignored — never commit it). Set up 2026-07-27.
